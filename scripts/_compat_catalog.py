...
def add(
    symbol: str, group: str, disp: str, milestone: str = "", note: str = ""
) -> None:
    CATALOG.append((symbol, group, disp, milestone, note))


def add_many(
    group: str, disp: str, milestone: str, members: list[str], note: str = ""
) -> None:
    for m in members:
        add(
            f"{group}.{m}" if group not in ("module", "constants") else m,
            group,
            disp,
            milestone,
            note,
...
add(
    ...
    OUT_OF_SCOPE,
    "post-v1",
    "journalling deferred (PRD §3.2 #5)",
)
# Pages — access / layout
add_many(
    "Document", IMPLEMENTED, "M1", ["load_page", "__getitem__", "pages", "page_count"]
)
add_many(
    "Document", IMPLEMENTED, "M3", ["new_page", "insert_pdf", "delete_page", "select"]
)
...
add(
    ...
    IMPLEMENTED,
    "M5",
    "image / PDF inputs (Pixmap / Document / bytes / path) via convert_to_pdf + insert_pdf; non-image non-PDF raises",
)
add(
    "Document.layout",
    "Document",
...
add(
    ...
    IMPLEMENTED,
    "M3",
    "xref of catalog /Metadata XML stream (0 if none)",
)
...
add(
    ...
    IMPLEMENTED,
    "M4",
    "/AcroForm /SigFlags int, -1 when no form",
)
...
add(
    ...
    IMPLEMENTED,
    "M1",
    "cross-reference revision count: startxref/`/Prev`-chain length minus the linearized first-page section (matches fitz exactly)",
)
add(
    ...
    IMPLEMENTED,
    "M4",
    "/AcroForm /NeedAppearances get/set, None when no form",
)
add_many(
    "Document",
    IMPLEMENTED,
    "M4",
    ["FormFonts"],
    "list of /AcroForm /DR /Font resource key names",
)
...
add(
    ...
    IMPLEMENTED,
    "M5",
    "image inputs converted to a 1-page PDF (fitz.open + convert_to_pdf); non-image raises PdfUnsupportedError",
)
add_many(
    "Document",
    IMPLEMENTED,
...
add_many(
    ...
    "M4",
    ["embfile_upd"],
    "update embedded-file content/names/desc in place + /Params /ModDate",
)
add_many(
    "Document",
    IMPLEMENTED,
    "M5",
    ["extract_font"],
    "embedded /FontFile* program + (basefont, ext, type, buffer) — byte-for-byte vs fitz across cff/ttf/cid/otf",
)
...
add(
    ...
    IMPLEMENTED,
    "M3",
    "URI fragment / named-destination → page index",
)
add_many(
    "Document",
    IMPLEMENTED,
...
add(
    ...
    IMPLEMENTED,
    "M3",
    "all /Dests names → {page, to, zoom, dest} (fitz-shaped)",
)
...
add(
    "Document.get_page_label",
    "Document",
    IMPLEMENTED,
    "M3",
    "delegates to the /PageLabels parser (Page.get_label)",
)
add(
    ...
    IMPLEMENTED,
    "M3",
    "writes /Root /PageLabels number tree",
)
...
add(
    ...
    IMPLEMENTED,
    "M5",
    "full-page render (delegates to Page.get_pixmap)",
)
add(
    ...
    IMPLEMENTED,
    "M2",
    "per-page XObject inventory (Form + Image)",
)
add_many(
    "Document",
    IMPLEMENTED,
...
add(
    ...
    IMPLEMENTED,
    "M6",
    "image pages (M5) + full vector-page render (M6d)",
)
add_many(
    "Page",
    IMPLEMENTED,
    "M6",
    ["get_displaylist"],
    "records the ordered render-op stream (M6d)",
)
add(
    "Page.get_svg_image", "Page", IMPLEMENTED, "M7", "page → standalone SVG string (M7)"
)
add(
    "Page.run",
    "Page",
...
add(
    ...
    IMPLEMENTED,
    "M4",
    "place another PDF page as a Form XObject (n-up/stamp/watermark)",
)
add_many(
    "Page",
    IMPLEMENTED,
    "M4",
    ["write_text"],
    "renders one or more TextWriter objects (direct draw / show_pdf_page compose)",
)
add_many(
    "Page",
    DEFERRED,
...
add(
    ...
    OUT_OF_SCOPE,
    "post-v1",
    "HTML/CSS engine out of scope (PRD §3.2 #2)",
)
# Annotations
add_many(
    "Page",
...
add_many(
    ...
    "M4",
    ["add_caret_annot"],
    "/Caret insertion marker (blue, fitz device rect + /AP)",
)
# Widgets / forms
...
add_many(
    ...
    "M4",
    ["add_widget"],
    "authors a /Widget field + registers /AcroForm (text/checkbox/combo/list); oracle round-trips",
)
...
add_many(
    ...
    "M3",
    ["set_contents"],
    "point /Contents at a stream xref (validated)",
)
...
add(
    ...
    IMPLEMENTED,
    "M8",
    "camelCase alias of get_textpage_ocr",
)
...
add(
    ...
    OUT_OF_SCOPE,
    "M6",
    "annot rasterization needs M6 renderer",
)
...
add(
    "Font.css_for_pymupdf_font",
    "Font",
    OUT_OF_SCOPE,
    "post-v1",
    "Story-only; HTML engine out of scope",
)
...
add_many(
    ...
    IMPLEMENTED,
    "M3",
    ["get_pdf_now", "get_pdf_str"],
    "PDF date/string formatting, fitz-exact",
)
add_many(
    "module",
    IMPLEMENTED,
    "M5",
    ["image_profile"],
    "raster header profile dict (width/height/xres/yres/colorspace/bpc/ext/cs-name/transform)",
)
add_many(
    "module",
    IMPLEMENTED,
...
add_many(
    ...
    "M7",
    ["find_tables"],
    "table detection via Page.find_tables (M7)",
)
add_many(
    "module",
    OUT_OF_SCOPE,
...
add(
    "Tools.mupdf_raw_access",
    "Tools",
    OUT_OF_SCOPE,
    "out-of-scope",
    "raw mupdf.* access raises PdfUnsupportedError (PRD §3.6)",
)
...
add(
    "constants.UCDN_SCRIPT",
    "constants",
    OUT_OF_SCOPE,
    "M6",
    "full shaping out of scope (PRD §3.2 #10)",
)
add(
    "constants.PdfUnsupportedError_catchall",
    "constants",
    OUT_OF_SCOPE,
    "M1",
...
_BATCH34_IMPLEMENTED = {
    ...
    "Document.xref_copy",
    "Document.xref_is_font",
    "Document.xref_is_image",
    ...
    "TextPage.extractSelection",
    "TextPage.extractTextbox",
    "TextPage.search",
    "TextPage.poolsize",
    "TextWriter.TextWriter",
    "TextWriter.append",
    "TextWriter.appendv",
    "TextWriter.clean_rtl",
    "TextWriter.color",
    "TextWriter.fill_textbox",
    "TextWriter.last_point",
    "TextWriter.opacity",
    "TextWriter.text_rect",
    "TextWriter.write_text",
    "Tools",
    "Tools.fitz_config",
    "Tools.gen_id",
    "Tools.glyph_cache_empty",
...
def render_toml() -> str:
    ...
    lines.append(
        "#   The machine-readable disposition matrix for every public symbol in the"
    )
    lines.append(
        "#   pinned PyMuPDF baseline (PRD §7 / §9.5). Every PyMuPDF symbol not present"
    )
    lines.append(
        "#   here is out-of-scope by default and must raise PdfUnsupportedError (never"
    )
    lines.append(
        "#   AttributeError). The compat-symbol-guard fails CI if any baseline symbol"
    )
    lines.append(
        "#   (compat/compat-baseline.txt) is missing an entry below — forcing an"
    )
    lines.append(
        "#   explicit disposition for any newly-surfaced API (baseline evolution, §17.2)."
    )
    lines.append("#")
    ...
    lines.append("#")
    lines.append(
        "# Generated from scripts/_compat_catalog.py — regenerate, do not hand-edit."
    )
    lines.append("")
    lines.append("[meta]")
