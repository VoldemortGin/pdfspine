...
`PYENC-*` exercise the native ``oxipdf`` package; the deprecated PyMuPDF aliases
...
def _open(markers: list[str]) -> "oxipdf.Document":
    return oxipdf.open(stream=multi_page_pdf(markers))
...
def test_pytoc_003_level_jump_raises():
    doc = _open(["AAA", "BBB"])
    with pytest.raises(oxipdf.PdfError):
...
def test_pymerge_001_insert_pdf():
    ...
    re = oxipdf.open(stream=dst.tobytes())
...
def test_pyedit_001_delete_select_reopen():
    ...
    doc2.select([2, 0])
    re2 = oxipdf.open(stream=doc2.tobytes())
...
def test_pylink_001_get_and_insert():
    doc = _open(["AAA", "BBB"])
    page = doc[0]
    page.insert_link({"kind": 2, "from": (10, 10, 100, 30), "uri": "https://oxipdf.dev"})
    re = oxipdf.open(stream=doc.tobytes())
    links = re[0].get_links()
    assert len(links) == 1
    assert links[0]["kind"] == 2
    assert links[0]["uri"] == "https://oxipdf.dev"
    assert isinstance(links[0]["from"], oxipdf.Rect)
...
def test_pylabel_001_get_label():
    ...
    doc = oxipdf.open(stream=_build_pdf(objects, root=1))
...
def test_pyenc_001_aes256_roundtrip():
    ...
    data = doc.tobytes(encryption=oxipdf.PDF_ENCRYPT_AES_256, user_pw="")
    re = oxipdf.open(stream=data)
...
def test_pyenc_002_owner_wrong_user():
    doc = _open(["AAA"])
    data = doc.tobytes(
        encryption=oxipdf.PDF_ENCRYPT_RC4_128, user_pw="theuser", owner_pw="theowner"
    )
    re = oxipdf.open(stream=data)
    assert re.is_encrypted
    assert re.authenticate("wrong") is False
    re2 = oxipdf.open(stream=data)
...
def test_pyfitz_edit_aliases():
    ...
    assert fitz.PDF_ENCRYPT_AES_256 == oxipdf.PDF_ENCRYPT_AES_256
