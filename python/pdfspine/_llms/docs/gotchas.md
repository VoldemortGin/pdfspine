...
- **abi3 wheel，仅支持 CPython 3.14**（`requires-python = ">=3.14,<3.15"`，maturin `features = ["pyo3/abi3-py311", ...]`）。abi3 的 3.11 ABI 下限是 `Pixmap` 零拷贝 buffer 协议（`bf_getbuffer`/`bf_releasebuffer` 稳定 ABI 槽，CPython 3.11 才有）所要求；安装门禁由 `requires-python` 元数据承担。
