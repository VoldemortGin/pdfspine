...
- **abi3 wheel，要求 CPython ≥ 3.11**（`requires-python = ">=3.11"`，maturin `features = ["pyo3/abi3-py311", ...]`）。abi3 的 3.11 ABI 下限是 `Pixmap` 零拷贝 buffer 协议（`bf_getbuffer`/`bf_releasebuffer` 稳定 ABI 槽，CPython 3.11 才有）所要求；安装下限同为 3.11，由 `requires-python` 元数据声明。
...
- out-of-scope（66 个，v1 不做）符号调用时抛 `PdfUnsupportedError`。
- 最大的 out-of-scope 块：`Story` / `Xml` / `Archive`（HTML/CSS → PDF 排版引擎，整块不做）。还有部分渲染期 `Tools` 旋钮、Widget JavaScript 钩子、数字签名**创建**。
- 当前 deferred 为 0；`Page.run` / `DisplayList.run` 使用本项目 ReplayDevice，不接受原生 FzDevice2 句柄。参见 replay.md。
...
- **覆盖率不是 100%**：当前为 91.4%（703/769）。迁移前用 `COMPAT.toml` 核对你依赖的符号。
...
- 链接渲染为蓝字 **并带可点击的 `/Link` 注解**（`links=True` 默认）：外链 → URI；`#anchor` → GoTo 目标标题。锚点用 **GitHub 风格 slug**（`## My Heading` → `#my-heading`；重名加 `-1`/`-2`；`{#id}` 显式 id 会**替换**自动 slug，此后 `#my-heading` 不再命中该标题）；匹配不到的锚点**静默不写注解**，不报错。
- 书签树（`toc=True` 默认）来自**全部**标题（含引用块 / 列表内的）；`#` 直接跳到 `###` 会被归一化成一级嵌套（`get_toc()` 里 level 是 1→2，不是 1→3）。不要指望 `set_toc` 那样的层级报错。
