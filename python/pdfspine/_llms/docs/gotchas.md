...
- deferred（2 个，计划做）和 out-of-scope（66 个，v1 不做）符号调用时抛 `PdfUnsupportedError`。
- 最大的 out-of-scope 块：`Story` / `Xml` / `Archive`（HTML/CSS → PDF 排版引擎，整块不做）。还有部分渲染期 `Tools` 旋钮、Widget JavaScript 钩子、数字签名**创建**。
- 已知 deferred（全部 2 个）：`Page.run`；`DisplayList.run`。
...
- **覆盖率不是 100%**：当前为 91.2%（701/769）。迁移前用 `COMPAT.toml` 核对你依赖的符号。
