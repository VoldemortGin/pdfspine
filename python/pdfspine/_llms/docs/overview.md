...
- OCR **引擎代码已编进 wheel**（纯 Rust PaddleOCR，PP-OCRv4；以及 Tesseract 适配器）。
- 但 **~16MB ONNX 模型不内嵌**，作为独立数据包 `pdfspine-ocr-models`（import 名 `pdfspine_ocr_models`）发布，由 extra **`pip install pdfspine[ocr]`** 拉入。`[all] == [ocr]`。
- 运行时模型解析顺序（见 `document.py::_ensure_ocr_models_env`）：
  1. 环境变量 `PDFSPINE_OCR_MODELS`（用户显式覆盖）；
  2. 已安装的 `pdfspine_ocr_models` 伴随包；
  3. 源码树内 `crates/pdf-ocr/models`（开发回退）；
  4. 都没有 → 抛 `PdfUnsupportedError`，提示 `pip install pdfspine[ocr]`。
...
明确不做：数字签名**创建**；HTML/CSS→PDF（Story/Xml/Archive）；部分渲染期旋钮。
