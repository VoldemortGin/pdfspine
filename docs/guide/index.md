...
This guide walks through using oxide-pdf for real work, from installation to the
individual feature areas. Every code block uses the **actual** public API.

## Two import styles

oxide-pdf ships two equivalent entry points:

```python
import oxide_pdf          # the native, idiomatic package
# or
import fitz               # the PyMuPDF compatibility shim
```

Both expose the same `open()`, `Document`, `Page`, `Pixmap`, and geometry
classes. Use `import oxide_pdf` for new code; use `import fitz` to run existing
...
| [Command-line interface](cli.md) | The planned `oxide-pdf` CLI. |
...
!!! note "Alpha status"
    oxide-pdf is pre-1.0 and under active development. The implemented surface is
    substantial but accuracy validation against a real PDF corpus is ongoing.
    Methods that are not yet implemented raise `oxide_pdf.PdfUnsupportedError`
