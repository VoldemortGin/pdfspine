...
`Page.run(dw,m)` and `DisplayList.run(dw,m,area)` return None. Callback,
TextPage append and RGB/RGBA drawing targets are available. These two catalog
entries are implemented under this explicitly normalized extension contract,
not native-device parameter compatibility. DisplayList accumulation is not supported.
...
At slice1 the two catalog entries remained deferred behind a temporary runtime
transition check. Slice3 removes that exception; the complete combined gate covers
...
no user callbacks; each run returns None. No native device ABI parity is claimed.
...
## RGB/RGBA target adapter (slice 3)

`ReplayDevice.for_pixmap(target)` paints onto the existing DeviceRGB Pixmap,
with or without alpha. Gray/CMYK and foreign targets are rejected. Target identity,
geometry, origin, DPI and previously exported read-only buffers are preserved.
DPI remains metadata: m alone controls scaling. Final device coordinates are
translated by minus target origin to address local pixels. Area uses the shared
conservative operation selector and never becomes an extra raster clip. Selected
operations replay together through the existing renderer/clip stack/font cache.

Public samples follow this project's existing straight RGB(A) renderer convention;
set_pixel writes those raw bytes and set_alpha does not rescale channels. Samples
are imported with tiny-skia's standard premultiply. A final premultiplied pixel
that equals its initial value retains the exact original straight bytes, including
hidden RGB at alpha0 and low-alpha quantization details. This detects observable
raster changes, not geometric touch, and does not use coarse operation bboxes as
dirty masks. Changed pixels use the existing Canvas demultiply conversion; no new
blend algorithm is introduced. An entirely unchanged result retains its old Arc.
This is not a claim of byte parity with native MuPDF's premultiplied pixel model.

The implementation claims a nonblocking per-target replay lease, snapshots the
sample Arc and metadata under a short borrow, releases that borrow for raster
work, and commits once. Two devices cannot concurrently replay into the same
Pixmap. Existing ordinary mutators retain their behavior; if they change the
samples or metadata during staging, commit raises RuntimeError and preserves the
intervening change. Old memoryviews keep their original Arc and bytes. A propagated
error leaves target bytes/metadata untouched and releases the lease. Legal zero-
dimension Pixmaps (for example from warp) are validated unchanged no-ops; this does
not change the ordinary Pixmap constructor's zero-dimension rejection.

Best-effort source degradation is inherited from the renderer: some undecodable
images/functions are skipped rather than raised. This adapter does not turn those
existing skips into strict failures or claim them as rollback tests. A private Rust
test injects failure only after actual selected painting, through the production
staging helper, without a public failure switch. Separate commit/lease tests cover
error results and lost-update prevention. Other tests cover all 65,536 channel/alpha
pairs for unchanged-byte preservation, straight source-over math, a full-bbox path
hole, old exported views, target origin with rotation/affine matrix, area selection,
and frozen embedded font/image streams after source edits and closure.
