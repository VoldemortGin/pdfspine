...
| ABI | **abi3-py311** → ONE wheel per (OS, arch); install gated to CPython **3.14** by `requires-python` | DONE |
| `requires-python` | `>=3.14,<3.15` | DONE |
...
Python 3.14 on Linux, macOS, and Windows. Confirm the release commit
...
- `pyproject.toml` classifiers: `Development Status :: 3 - Alpha`,
  ...
  Windows), per-minor Python (3.14), extra Topic classifiers.
...
the sdist on Python 3.14. Verify the TestPyPI dry run as an additional check:
