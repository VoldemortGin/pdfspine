...
| ABI | **abi3-py311** → ONE wheel per (OS, arch), CPython **≥ 3.11** (floor set by `requires-python`) | DONE |
...
Python 3.11–3.14 range on Linux, macOS, and Windows. Confirm the release commit
...
- `pyproject.toml` classifiers: `Development Status :: 3 - Alpha`,
  ...
  Windows), per-minor Python (3.11/3.12/3.13/3.14), extra Topic classifiers.
