...
| `WORDS-015` | e2e govdocs `Tf 1`/`Tm 8` label with `-332.7` `TJ` word gaps + `0.2219 Tc` dot leader + figures → `["Under","5","years", "."×31, "88", "6.1"]` — a leader joins no letters, so its `Tc` is never deducted and every dot stays its own word | PRD §8.6.2 | green |
...
| ID | feature | spec ref | status |
|---|---|---|---|
| `ONNX-001` | `OnnxOptions` validation (types, ranges, channel order, providers, `layout_variant` casefold + allowed set, `layout_size` int-or-None, the five cell post-processing switches `cell_postprocess` / `band_word_assignment` / `merge_symbol_columns` / `strip_dot_leaders` / `verify_spans` bool-only) and `from_mapping` unknown-key rejection; defaults `layout_threshold=0.5`, `layout_nms_iou=0.6`, `cell_postprocess` / `merge_symbol_columns` / `strip_dot_leaders` `True` and `band_word_assignment` / `verify_spans` `False` | pdfspine vision extension | green |
...
| `ONNX-021` | `skip_layout=True` bypasses the layout detector and feeds the page-space `clip=` straight to SLANet as the sole table region (gold-crop TSR-only scoring, ADR 0002); `clip=None` raises `ValueError` | ONNX end-to-end contract | green |

| `ONNX-016` | `_strip_dot_leaders` removes `. . . .` / `......` leader tokens and lone leading / trailing dots, keeps `1,234.56`, `U.S. Treasury` and a lone `.` inside the line; `_apply_cell_text` honours `strip_leaders` both ways | text-layer fill contract | green |
| `ONNX-017` | `_cell_bands` core / band split and `_band_index` "gap goes to the band below" rule; a wrapped row label's first line is stolen by the row above under `_assign_words` and kept by `_assign_words_bands` | text-layer fill contract | green |
