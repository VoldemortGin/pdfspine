...
| `ONNX-002` | model path resolution: explicit path > `PDFSPINE_ONNX_MODELS` > bare filename, per-variant default file name; missing model file and missing runtime raise `PdfUnsupportedError` with the variant's file name + download URL / `pdfspine[onnx]` hint | ONNX runtime contract | green |
...
| `ONNX-006` | `_decode_layout` reads RT-DETR `[cls, score, x0, y0, x1, y1(, read_order)]` rows in original-image pixels (clamped), skips padding (`cls < 0`), sub-threshold and zero-area rows, truncates to `count`, maps model classes to pdfspine labels per variant (`label` + `raw_label`, V3 `footnote`/`vision_footnote`), same-class NMS vs `_nms(per_class=False)`; class-id order pinned; labels from metadata `names`/`character` | PP-DocLayout post-processing | green |
...
| `ONNX-012` | `_grid_boxes` derives row/column bands from cells; runtime cache is keyed by model paths + providers + layout variant (L and V3 runtimes carry their own labels, label map, model file and input edge), evicts at 4 entries and `clear_model_cache` empties it | ONNX runtime contract | green |
...
| `ONNX-014` | `_layout_input` plain square resize, RGB `[0, 1]` NCHW float32 without padding or mean/std, `scale_factor=(size/h, size/w)`; `_input_edge` reads the static square edge of the `image` input only; `_variant_from_name` / `_layout_variant` precedence (option > file name > default); `detect_layout` feeds `image`/`im_shape`/`scale_factor` by name, honours `layout_size`, takes the box count from the second output and rejects models without an `image` input | PP-DocLayout pre-processing | green |
