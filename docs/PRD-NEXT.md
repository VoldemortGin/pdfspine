...
- [ ] **5. `pdf-typeset` next increments.**
  ...
  - [x] **Caller-resolved solid paragraph connections** (unreleased).
    Additive checked-only overlay with structural paths and typed errors;
    existing model fields and four checked return types remain unchanged.
    Same-page and page-break reservations share line metrics; container clones
    preserve paths. Twelve focused tests, prior 11-PDF byte equality and all
    13 reference pages at 100dpi pass. Implementation `267961b` passes the complete
    five-stage gate: **2,046 Rust / 1,535 Python passed, 68 existing skips**,
...
| C4 | §D: "text already at parity, diminishing returns" | Read as overstated off a **stale 06-16-morning report** (PMC 0.08–0.44, born 0/6); the multi-column engine landed 06-16 PM and **✅ P3-2 verified parity** (2026-06-20; numbers restated 2026-09-04): the 0.965/0.995 PMC order figure was computed over 12 documents, 5 of which are mis-paired — PLoS Biology published one Research Digest *section* PDF that the PMC OA package attaches to every synopsis in it, so those PDFs are a strict superset of their own XML ground truth. Those 5 are now quarantined out of the aggregates (`run_gt.py::pairing_verdict`). Over the 7 clean documents PMC order is **0.939/0.996** vs fitz **0.961/0.996**; 3 of the 4 nominal losses are −0.000, i.e. ties at three decimals. Parity therefore holds on every clean document except PMC212689 (0.599 vs 0.749 → P3-1r). born 0.996 vs fitz. | `conformance/gt/GT-REPORT-pmc.md`, `GT-REPORT-born.md` |
