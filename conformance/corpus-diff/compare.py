"""差分：对齐 reference 与 candidate 的词序列，统计 over-split / under-split，并关联字体特征。

用法：python compare.py [reference-label candidate-label [summary-name]]。
"""
...
def segments(
    ...
    for c in common[1:]:
        sa, sb = [], []
        while oa < c:
            sa.append(words_a[ia])
            oa += len(words_a[ia])
            ia += 1
        while ob < c:
            sb.append(words_b[ib])
            ob += len(words_b[ib])
            ib += 1
        out.append((sa, sb))
    return out
...
def align_page(wa: list[str], wb: list[str]) -> dict[str, Any]:
    ...
    # 词的字符起止区间
    def spans(ws: list[str]) -> list[tuple[int, int]]:
...
def font_flags(f: dict[str, Any]) -> list[str]:
    flags = []
    st = f.get("Subtype")
...
def main() -> None:
    ...
    parser.add_argument("reference", nargs="?", default="fitz")
    parser.add_argument("candidate", nargs="?", default="pdfspine")
    parser.add_argument("summary", nargs="?", default="summary.json")
    args = parser.parse_args()
    out_a, out_b = HERE / "out" / args.reference, HERE / "out" / args.candidate
    files = [
        line for line in (HERE / "corpus.txt").read_text().split("\n") if line.strip()
    ]
    per_file: list[dict[str, Any]] = []
    per_page: list[dict[str, Any]] = []
    errs: dict[str, list[tuple[str, str]]] = {args.reference: [], args.candidate: []}
    ...
    for idx, path in enumerate(files):
        ...
        if a["error"]:
            errs[args.reference].append((path, a["error"][:120]))
        if b["error"]:
            errs[args.candidate].append((path, b["error"][:120]))
        fstat: Counter[str] = Counter()
        fex_over, fex_under = [], []
        for pno, pa in a["pages"].items():
            pb = b["pages"].get(pno)
            if pb is None or "error" in pa or "error" in pb:
                if pb is not None and "error" in pb:
                    errs[args.candidate].append((f"{path}#p{pno}", pb["error"][:120]))
                continue
        ...
        per_file.append(
            {
                ...
                "path": path,
                **fstat,
                "over_examples": fex_over[:MAX_EXAMPLES],
    ...
    for r in top:
        r["font_detail"] = [
            font_index.get(r["path"], {}).get(str(x)) for x in r["fonts"]
        ]

    # 相关性：按页级 flag 是否出现，比较 over-split 率（over / n_fitz）
    corr: dict[str, dict[str, int | float]] = {}
    all_flags = set()
    for r in per_page:
    ...
    for g in sorted(all_flags):
        ...
        corr[g] = {
            "pages_with": len(with_f),
            "over_with": sum(r["over"] for r in with_f),
            "over_per_1k_words_with": round(rate(with_f), 2),
            "pages_without": len(without),
            "over_without": sum(r["over"] for r in without),
    ...
    (HERE / args.summary).write_text(json.dumps(summary, ensure_ascii=False, indent=1))

    print(f"files={len(per_file)} pages={len(per_page)}")
    print("totals:", dict(tot))
    print(
        f"errors {args.reference}:",
        len(errs[args.reference]),
        f"{args.candidate}:",
        len(errs[args.candidate]),
    )
    print("\n== top over-split pages ==")
    for r in top:
        print(
            f"{r['over']:4d} over ({r['over_alpha']} alpha) {r['under']:3d} under  ratio={r['ratio']} "
    ...
    for f in sorted(per_file, key=lambda x: -x["over"]):
        if f["over"] or f["under"]:
            print(
                f"{f['over']:5d} over ({f['over_alpha']} alpha/{f['over_punct']} punct) {f['under']:5d} under "
                f"{f['mixed']:4d} mixed pages={f['pages']} eq={f['content_equal']} {Path(f['path']).name}"
            )
    print("\n== font flag correlation ==")
    for g, c in corr.items():
        print(
            f"{g:28s} with: pages={c['pages_with']:4d} over={c['over_with']:5d} /1k={c['over_per_1k_words_with']:6.2f} "
