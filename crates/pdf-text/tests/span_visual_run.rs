...
#[test]
fn spanvis_002_small_rotation_splits_the_span() {
    ...
    // by sin 2° = 0.0349 of the size — well past `SPAN_LINEAR_TOL_FRAC`.
    let got = shape(
...
#[test]
fn spanvis_004_baseline_shift_splits_the_span() {
    // Down 1 pt at 12 pt = 0.083 of the size. A *downward* shift sets no
    // `SUPERSCRIPT` flag, so the old rule merged it; 1 pt is far inside the
...
fn spanvis_010_tolerances_scale_with_the_font() {
    // The same absolute 1 pt baseline drop that splits a 12 pt run
