...
pub struct Block {
    ...
    /// block's lines. Since reading-order stage 3 the block sequence is purely
    /// geometric ([`crate::layout`]'s XY-cut emission order) and `seq` only
    /// orders the lines *within* one region, so a block never straddles a
    /// column and co-baseline fragments keep their paint order. Image blocks
    /// default to `usize::MAX` and are appended after the text blocks.
    pub seq: usize,
}
