...
/// A shading color: 1–4 normalized components in `[0, 1]` matching the shading's
/// colorspace (Gray / RGB / CMYK).
#[derive(Clone, Debug, PartialEq)]
pub struct ShadingColor(pub Vec<f32>);

/// A minimal PDF function (types 0/2/3) driving a shading color ramp.
///
/// Only the function types shadings commonly use are supported; PostScript
/// calculator functions (type 4) are **deferred** (documented gap).
#[derive(Clone, Debug, PartialEq)]
pub enum PdfFunction {
    /// Type 2 — exponential interpolation between `c0` and `c1`: each output is
    ...
    /// Type 0 — sampled: 1-input, `n_outputs`-output table of `size` samples,
    /// linearly interpolated. `bits_per_sample` ∈ {1,2,4,8,16}.
    Sampled {
        /// The 1-input domain `[t0, t1]`.
        domain: [f32; 2],
        ...
        n_outputs: usize,
        /// The `[lo, hi]` input→sample-index encode range.
        encode: [f32; 2],
...
impl PdfFunction {
    /// Evaluates the function at input `t`, returning the output color.
    #[must_use]
    pub fn eval(&self, t: f32) -> ShadingColor {
        match self {
            ...
            PdfFunction::Stitching {
                ...
                // The sub-interval [lo, hi) of the domain for sub-function k.
    ...
    fn eval_sampled(
        ...
        if size == 0 || n_outputs == 0 {
            return ShadingColor(vec![0.0]);
        }
        let t = clamp(t, domain[0], domain[1]);
        // Map t -> sample-index space [encode[0], encode[1]], clamped to [0,size-1].
        let e = interpolate(t, domain[0], domain[1], encode[0], encode[1]);
        ...
        let read = |idx: usize, out: usize| -> f32 {
            let bit = (idx * n_outputs + out) * bits_per_sample as usize;
            read_bits(samples, bit, bits_per_sample as usize) as f32 / max
        };
        let out = (0..n_outputs)
            .map(|o| {
                let s0 = read(i0, o);
