...
fn wrap_off_keeps_one_line() {
    let mut e = ts();
    let m = e.measure_blocks(
        &[para("these words would surely wrap here", 12.0)],
        40.0,
        false,
    );
...
fn empty_paragraph_measures_nothing() {
    let mut e = ts();
    let m = e.measure_blocks(
        &[Block::Paragraph(ParaProps::new(), Vec::<Run>::new())],
        200.0,
        true,
    );
...
fn measure_text_box_matches_measure_blocks_at_box_width() {
    let mut e = ts();
    let blocks = vec![para(
        "some wrapping text here to fill several lines nicely",
        12.0,
    )];
    let rect = Rect {
        x0: 100.0,
    ...
    assert_eq!(
        via_box, via_blocks,
        "text box measures its content at rect width"
    );
...
fn measured_height_matches_text_box_layout() {
    ...
    let blocks = vec![para(
        "point consistency between measure and real layout here",
        12.0,
    )];
    ...
    assert_near(
        ...
        predicted,
        1.0,
        "measure height drives the anchor offset",
    );
}
